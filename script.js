...
window.goToPage = function(pageName) {
  ...
  if (pageName === 'codelab') initCodeLab();
  // AI Assistants hub
...
function initSharedRealtime() {
  ...
  sb.channel('public:code_lab_scores')
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'code_lab_completions' }, () => {
      ...
      if (board) openCodeLabLeaderboard();
      refreshCodeLabStatus();
    })
    .subscribe();
}
...
const CODE_LAB_CHALLENGES = [
  {
    ...
    language: 'html',
    files: {
      html: `<section class="card">\n  <h1>Debug Lab</h2>\n  <img src=""\n  <button onclick="document.body.classList.toggle('active')">Toggle</button>\n</section>`,
      ...
      javascript: `console.log('Profile card ready');`,
      java: ''
    },
  },
  {
...
let codeLabPracticeFiles = JSON.parse(localStorage.getItem('codeLabPracticeFiles') || 'null') || { ...CODE_LAB_PRACTICE_STARTER };
let codeLabAssets = JSON.parse(localStorage.getItem('codeLabAssets') || '[]');
...
function getDailyCodeLabChallenge() {
  const key = codeLabTodayKey();
  const seed = key.replace(/-/g, '').split('').reduce((sum, ch) => sum + Number(ch), 0);
  return CODE_LAB_CHALLENGES[seed % CODE_LAB_CHALLENGES.length];
}
...
function loadMonacoForCodeLab() {
  return new Promise((resolve) => {
    ...
    loader.src = 'https://cdn.jsdelivr.net/npm/monaco-editor@0.49.0/min/vs/loader.js';
    loader.onload = () => {
      window.require.config({ paths: { vs: 'https://cdn.jsdelivr.net/npm/monaco-editor@0.49.0/min/vs' } });
      window.require(['vs/editor/editor.main'], () => resolve(true), () => resolve(false));
    };
...
function registerCodeLabSuggestions() {
  ...
  const javaSuggestions = ['public class Main', 'public static void main(String[] args)', 'System.out.println', 'JFrame', 'JPanel', 'JButton', 'JLabel', 'JTextField', 'ActionListener', 'BorderLayout', 'FlowLayout'];
...
async function refreshCodeLabStatus() {
  ...
  const rows = data || [];
  const solvedToday = rows.some((row) => row.challenge_id === challenge.id && row.challenge_date === codeLabTodayKey());
  if (badge) {
...
function loadDailyCodeLabChallenge() {
  ...
  setCodeLabLanguage(challenge.language || 'html', false);
...
function renderCodeLabAssets() {
  ...
  box.innerHTML = codeLabAssets.map((asset, index) => `
    <div class="code-lab-asset">
      ${asset.type?.startsWith('image/') ? `<img src="${escapeHTML(asset.url)}" alt="">` : '<span class="code-lab-video-thumb">VIDEO</span>'}
      <div>
        <strong>${escapeHTML(asset.name)}</strong>
        <small>${escapeHTML(asset.type || 'asset')}</small>
      </div>
      <button onclick="copyCodeLabAssetSrc(${index})">Copy src</button>
      <button onclick="insertCodeLabAssetSrc(${index})">Insert</button>
    </div>
  `).join('');
}
...
window.initCodeLab = async function() {
  ...
  if (!codeLabInitialized) {
    ...
    const fallback = document.getElementById('code-lab-editor-fallback');
    if (fallback) fallback.addEventListener('input', () => saveActiveCodeLabFile());
  }
...
window.switchCodeLabMode = function(mode) {
  ...
  const active = document.getElementById(`code-lab-tab-${codeLabMode}`);
...
window.runCodeLab = async function() {
  saveActiveCodeLabFile();
  codeLabConsoleLines = [];
  if (codeLabLanguage === 'java' || (codeLabMode === 'daily' && getDailyCodeLabChallenge().language === 'java')) {
...
window.submitCodeLabChallenge = async function() {
  if (!currentUser?.username) return customAlert('Login first to claim debug points.');
  saveActiveCodeLabFile();
  const challenge = getDailyCodeLabChallenge();
  ...
  try {
    ...
    const { data, error } = await sb.rpc('class_app_claim_code_lab_point', {
    ...
    if (data === 'already_claimed') {
      ...
      showToast('CODE LAB point earned.');
    }
    refreshCodeLabStatus();
  } catch (error) {
    codeLabSetConsole(error.message, 'error');
...
window.openCodeLabLeaderboard = async function() {
  removeDynamicModal('code-lab-leaderboard-modal');
  document.body.insertAdjacentHTML('beforeend', `
    <div id="code-lab-leaderboard-modal" class="custom-modal-overlay blur-bg high-z" style="display:flex;">
      <div class="custom-modal-box contribution-modal-box border-blue">
        <button class="modal-close-btn" onclick="removeDynamicModal('code-lab-leaderboard-modal')">&times;</button>
        <h3 class="modal-title text-blue">CODE LAB Leaderboard</h3>
        <div class="contribution-board-list" id="code-lab-leaderboard-list">${createInlineLoader('Loading leaderboard...')}</div>
      </div>
    </div>
  `);
  const list = document.getElementById('code-lab-leaderboard-list');
  try {
    const { data, error } = await sb.rpc('class_app_code_lab_leaderboard');
    if (error) throw error;
    const rows = data || [];
    ...
    list.innerHTML = `<p class="lobby-open-empty">${escapeHTML(error.message)}</p>`;
  }
};

window.resetCodeLabWorkspace = function() {
  customConfirm('Reset the current CODE LAB workspace?', () => {
    if (codeLabMode === 'daily') loadDailyCodeLabChallenge();
    else {
      codeLabPracticeFiles = { ...CODE_LAB_PRACTICE_STARTER };
      codeLabFiles = { ...codeLabPracticeFiles };
      localStorage.setItem('codeLabPracticeFiles', JSON.stringify(codeLabPracticeFiles));
      setCodeLabLanguage('html', false);
    }
...
window.uploadCodeLabAssets = async function(event) {
  ...
  for (const file of files) {
    ...
    form.append('username', currentUser?.username || 'guest');
    try {
