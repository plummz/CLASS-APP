/* ============================================================
   SCRIPT.JS — My School Portfolio (Node.js Integrated)
   ============================================================ */
...
/* ============================================================
   CARD BUILDERS (Cleaned up to trigger Folders)
   ============================================================ */
...
function buildFolderCards(gridId, count, prefix) {
  const grid = document.getElementById(gridId);
  if (!grid) return;
  grid.innerHTML = '';
  ...
  for (let i = 1; i <= count; i++) {
    ...
    card.style.cursor = 'pointer';
    card.onclick = () => openFolderExplorer(`${prefix} ${i}`);
...
/* ============================================================
   ...
   ============================================================ */
let currentParentContext = null; // e.g., 'Math 0'
let currentFolderContext = null; // e.g., { id: '123', name: 'Midterms' }
        ...
        files.forEach(f => {
            ...
            <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; gap: 15px;">
                <div style="flex: 1; overflow: hidden;">
                    ...
                    <div style="color: gray; font-size: 11px;">Uploaded by: ${f.uploader} | ${new Date(f.createdAt).toLocaleDateString()}</div>
                </div>
                <div style="display: flex; gap: 10px;">
                    <button onclick="playOrOpenFile('${f.url}', '${safeName}')" class="btn-primary" style="padding: 8px 15px; font-size: 12px;">Open</button>
...
window.uploadFileToFolder = async function() {
    ...
    try {
        // 1. Upload file to server storage via Multer
        const uploadRes = await fetch(`${SERVER_BASE}/api/upload`, { method: 'POST', body: form });
        if(!uploadRes.ok) throw new Error("File upload failed");
        const uploadedData = await uploadRes.json();

        // 2. Save metadata to database assigned to currentFolderContext.id
...
window.playOrOpenFile = function(url, name) {
    const fullUrl = SERVER_BASE + url; // Need absolute URL since files are on server
    ...
    if (name.toLowerCase().endsWith('.mp3') || name.toLowerCase().endsWith('.wav')) {
        ...
        // Jump to Music Hub page if playing audio from elsewhere
...
window.startVisualizer = (audioElement) => {
  if (!window.audioCtx) {
    ...
    // IMPORTANT: Since audio is loaded via API, CORS is required. 
    // The HTML element <audio crossorigin="anonymous"> is set to handle this.
  ...
  function loop() {
    ...
    // Grab lower frequencies for the "bass bump" effect
  ...
  users
    ...
    .forEach((user) => {
      ...
      item.innerHTML = `
        ...
        <button onclick="openChat('private', '${user.username}')" class="btn-primary" style="padding:5px 10px; font-size:12px;">Chat</button>
      `;
...
window.sendMessage = function() {
  ...
  if (currentChat.type === 'private' && !currentChat.target) return alert('Select a private contact.');
