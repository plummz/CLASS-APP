...
(function () {
  ...
  async function requestSummary(type) {
    ...
    try {
      ...
      // Save to Notepad and to persistent Summary History
      await saveToNotepad(selectedFile.name, type, data.summary);
      saveSummaryHistory(selectedFile.name, data.summary);
  ...
  function saveSummaryHistory(sourceName, summaryText) {
    if (!window.authFetch) return;
    window.authFetch('/api/summary-history', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ source_name: sourceName || '', summary_text: summaryText || '' }),
    }).catch((e) => console.warn('[FileSummarizer] summary history save failed:', e.message));
  }
