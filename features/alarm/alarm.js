...
window.alarmModule = {
  ...
  triggerAlarm: function(alarm, index) {
    ...
    setTimeout(() => {
      this.alarms[index].triggered = false;
      this.saveAlarms();
    }, 60000);
  },
  addToAnnouncements: function(alarm) {
    ...
    if (window.shareAnnouncementPayload) {
      window.shareAnnouncementPayload({
        ...
        body: `Alarm triggered at ${alarm.time}. ${alarm.noteIndex !== null ? 'Linked to notepad reminder.' : ''}`,
        schedule: 'Alarm',
  ...
  addToCalendar: function(alarm) {
    ...
    if (window.calendarNotes && window.sb) {
      const note = `Alarm triggered: ${alarm.label} at ${alarm.time}`;
    ...
    page.innerHTML = `
      ...
      <div class="alarm-container">
        ...
        <div class="alarm-controls">
          <button class="alarm-btn" onclick="alarmModule.showForm()">+ Set Alarm</button>
          <button class="alarm-btn" onclick="alarmModule.toggleAllAlarms()">Toggle All</button>
        </div>

        <div class="alarm-list" id="alarm-list">
          ${this.alarms.length === 0 ? '<p style="color: #888; text-align: center;">No alarms set.</p>' : ''}
        </div>
        ...
        <div class="alarm-form" id="alarm-form" style="display: none;">
          ...
          <button onclick="alarmModule.hideForm()">Cancel</button>
        </div>

        <div class="alarm-notifications" id="alarm-notifications" style="display: none;">
          <h3>Recent Alarms</h3>
          <div id="alarm-notification-list"></div>
        </div>
      </div>
    `;
    ...
    const alarmsHtml = this.alarms.map((alarm, index) => `
      <div class="alarm-item ${alarm.active ? 'active' : ''}">
        ...
        <div class="alarm-item-actions">
          <button class="alarm-btn ${alarm.active ? 'active' : ''}" onclick="alarmModule.toggleAlarm(${index})">
            ${alarm.active ? 'ON' : 'OFF'}
          </button>
          <button class="alarm-btn" onclick="alarmModule.editAlarm(${index})">Edit</button>
          <button class="alarm-btn" onclick="alarmModule.deleteAlarm(${index})">Delete</button>
        </div>
      </div>
    `).join('');

    listEl.innerHTML = alarmsHtml || '<p style="color: #888; text-align: center;">No alarms set.</p>';
  },
  populateNoteOptions: function() {
    const select = document.getElementById('alarm-note');
    if (!select) return;

    // Get notes from notepad module
